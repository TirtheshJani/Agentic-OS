---
name: stellar-mk-audit
slug: stellar-mk-audit
description: >-
  Solo arXiv paper auditing tree-based stellar MK classifiers via causal
  masked-line ablation (astro-ph.IM).
status: active
branch: physics
path: 'c:/Users/TJ/Documents/GitHub/stellar-mk-audit'
capabilities:
  - research
  - coding
  - physics
agent: Plan
repo: 'https://github.com/TirtheshJani/stellar-mk-audit'
created: '2026-05-15'
crew: []
runtime-default: claude-code
---

# stellar-mk-audit

12-week phase plan in `project_plan.md`. Phase gates enforced via 4-agent loop (Planner/Physicist/Coder/Reviewer). Project-level CLAUDE.md is at `c:/Users/TJ/Documents/CLAUDE.md` and applies to all sibling repos in that folder.
