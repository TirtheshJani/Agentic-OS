...
import {
  Terminal, RefreshCw, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, GitBranch, Clock, Wrench, Star,
  Archive, ArchiveRestore, FileText, Save, Eraser, Filter, Bot,
} from 'lucide-react';
import {
  fetchCodexCliSessions, fetchCodexCliSessionStats, triggerCodexCliScan, updateCodexCliSessionMeta,
} from '../api/codexCliSessions';
import type { CodexCliSessionSort, CodexCliSessionSummary } from '../api/codexCliSessions';
      ...
      <div className="flex items-end gap-0.5" style={{ height: 56 }}>
        {recent.map((d) => (
          <div
            ...
            style={{
              height: Math.max(3, Math.round((d.sessions / max) * 48)),
...
type SessionMetaPatch = Partial<Pick<CodexCliSessionSummary, 'starred' | 'archived' | 'note'>>;
...
function SessionRow({
  ...
  const topTools = Object.entries(session.tool_calls)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 5);
  ...
  return (
    <>
      <div
        className={cn(
          'grid gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors cursor-pointer',
          session.archived && 'opacity-70',
        )}
        style={{
          gridTemplateColumns: '28px minmax(0,2fr) 1fr 1fr 80px 80px 24px',
      ...
      >
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onMetaPatch(session.session_id, { starred: !session.starred });
          }}
      ...
      {expanded && (
        ...
        >
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
            ...
            {session.source && (
              <div>
                <span className="font-medium uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>Source</span>
                <div className="font-mono mt-0.5" style={{ color: 'var(--text-secondary)' }}>{session.source}</div>
              </div>
            )}
            {session.cli_version && (
              <div>
                <span className="font-medium uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>CLI Version</span>
                <div className="font-mono mt-0.5" style={{ color: 'var(--text-secondary)' }}>{session.cli_version}</div>
              </div>
            )}
            {session.updated_at && (
              <div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={2000}
              placeholder="Add context, follow-up, or ownership notes for this session..."
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => navigate(`/codex-sessions/${session.session_id}`)}
            >
              <Terminal size={11} /> View full session
            </button>
          </div>
        </div>
      )}
...
const PAGE_SIZE = 20;
const SORT_OPTIONS: { value: CodexCliSessionSort; label: string }[] = [
...
export function CodexCliSessionsPage() {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [projectFilter, setProjectFilter] = useState('all');
  const [modelFilter, setModelFilter] = useState('all');
  const [sort, setSort] = useState<CodexCliSessionSort>('newest');
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: queryKeys.codexCliSessionStats(),
    queryFn: () => fetchCodexCliSessionStats(30),
    staleTime: 60_000,
  });

  const { data, isLoading } = useQuery({
    queryKey: queryKeys.codexCliSessions(
      page,
      search,
      projectFilter,
      modelFilter,
  ...
  const { mutate: rescan, isPending: scanning } = useMutation({
    mutationFn: triggerCodexCliScan,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.codexCliSessionStats() });
      queryClient.invalidateQueries({ queryKey: queryKeys.codexCliSessions() });
    },
  });
  ...
  const { mutate: patchSessionMeta, isPending: isMetaSaving } = useMutation({
    mutationFn: ({ sessionId, patch }: { sessionId: string; patch: SessionMetaPatch }) =>
      updateCodexCliSessionMeta(sessionId, patch),
    onMutate: ({ sessionId }) => {
      setPendingMetaSessionId(sessionId);
    },
    onSettled: () => {
      setPendingMetaSessionId(null);
      queryClient.invalidateQueries({ queryKey: queryKeys.codexCliSessions() });
      queryClient.invalidateQueries({ queryKey: queryKeys.codexCliSession() });
    },
  });
  ...
  const avgDurStr = avgDur != null ? durationLabel(avgDur) : null;
  ...
  return (
    <div className="p-6 flex flex-col h-full gap-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Codex Sessions</h1>
        {data && (
        ...
        <button
          className={cn(
            'ml-auto flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-all hover:bg-white/10',
        ...
        <button
          ...
          onClick={() => {
            const ids = selectedForAgent.size > 0 ? Array.from(selectedForAgent).join(',') : '';
            navigate(`/agent-library/new${ids ? `?sessions=${ids}` : ''}`);
          }}
        >
          <Bot size={12} />
          Build Agent{selectedForAgent.size > 0 ? ` (${selectedForAgent.size})` : ''}
        </button>
      </div>
      ...
      <div className="card px-3 py-3 space-y-2">
        <div className="grid grid-cols-1 gap-2 lg:grid-cols-[minmax(0,1fr)_160px_160px_130px_120px_auto]">
          ...
          <select
            ...
            className="px-2 py-1.5 text-xs rounded-md bg-transparent"
            style={{ border: '1px solid var(--border)', color: 'var(--text-secondary)' }}
          >
          ...
          <select
            value={sort}
            onChange={(e) => { setSort(e.target.value as CodexCliSessionSort); setPage(1); }}
            className="px-2 py-1.5 text-xs rounded-md bg-transparent"
            style={{ border: '1px solid var(--border)', color: 'var(--text-secondary)' }}
          >
          ...
          <input
            ...
            max={200}
            value={minTools}
            onChange={(e) => { setMinTools(Math.max(0, Number(e.target.value) || 0)); setPage(1); }}
            className="px-3 py-1.5 text-xs rounded-md bg-transparent"
            style={{ border: '1px solid var(--border)', color: 'var(--text-secondary)' }}
            placeholder="Min tools"
          />

          <button
            className="px-3 py-1.5 rounded-md text-xs hover:bg-white/10 transition-all disabled:opacity-40"
            style={{ color: 'var(--text-secondary)', border: '1px solid var(--border)' }}
            onClick={() => {
              setSearch('');
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className="flex items-center gap-1 px-2.5 py-1 rounded text-xs hover:bg-white/10 transition-all"
            style={{
              border: '1px solid var(--border)',
              color: starredOnly ? '#fbbf24' : 'var(--text-secondary)',
              background: starredOnly ? 'rgba(251,191,36,0.08)' : undefined,
            }}
          ...
          <button
            ...
            style={{
              ...
              background: includeArchived ? 'rgba(255,255,255,0.06)' : undefined,
            }}
            ...
            {(!data || data.items.length === 0) ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 py-16">
                ...
                <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>
                  {hasFilters ? 'No sessions match the current filters.' : 'No Codex sessions found.'}
                </p>
                <button
