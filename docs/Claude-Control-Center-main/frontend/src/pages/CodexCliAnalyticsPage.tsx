...
import { fetchCodexCliSessionStats, triggerCodexCliScan } from '../api/codexCliSessions';
...
function HorizontalBar({ name, count, max }: { name: string; count: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs font-mono w-36 truncate flex-shrink-0" style={{ color: 'var(--text-secondary)' }} title={name}>
        {name}
      </span>
    ...
    <div className="card px-4 py-3">
      ...
      <div className="flex items-end gap-0.5" style={{ height: 60 }}>
        {data.slice(-30).map((d) => (
          <div
            ...
            style={{
              height: Math.max(3, Math.round((d.sessions / max) * 52)),
      ...
      <div className="flex justify-between mt-1">
        <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>
          {data[Math.max(0, data.length - 30)]?.date}
        </span>
      ...
      <div className="flex items-end gap-px" style={{ height: 40 }}>
        {data.map((d) => (
          <div
            ...
            className="flex-1 rounded-sm"
            style={{
              height: Math.max(2, Math.round((d.count / max) * 36)),
              background: 'var(--accent)',
              opacity: 0.6,
            }}
...
export function CodexCliAnalyticsPage() {
  const [days, setDays] = useState<DaysOption>(30);
  const queryClient = useQueryClient();

  const { data: stats, isLoading } = useQuery({
    queryKey: queryKeys.codexCliAnalytics(days),
    queryFn: () => fetchCodexCliSessionStats(days),
    staleTime: 60_000,
  });

  const { mutate: rescan, isPending: scanning } = useMutation({
    mutationFn: triggerCodexCliScan,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: queryKeys.codexCliAnalytics() }),
  });

  const overview = stats?.overview;
  const avgDur = overview?.avg_duration_seconds;
  const avgDurStr = avgDur != null
    ? avgDur < 60 ? `${Math.round(avgDur)}s` : `${Math.round(avgDur / 60)}m`
    : null;
  ...
  return (
    <div className="p-6 flex flex-col h-full gap-5 overflow-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Codex Analytics</h1>
        ...
        <div className="flex gap-0.5 p-0.5 rounded-md" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)' }}>
          {DAYS_OPTIONS.map((opt) => (
            <button
              ...
              className={cn('px-2.5 py-1 text-xs rounded transition-all')}
              style={{
                background: days === opt.value ? 'rgba(255,255,255,0.08)' : undefined,
                color: days === opt.value ? 'var(--text-primary)' : 'var(--text-secondary)',
                fontWeight: days === opt.value ? 500 : undefined,
              }}
            >
              {opt.label}
            </button>
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          {stats.tools.top_tools.length > 0 && (
            <div className="card px-4 py-3">
              ...
              <div className="space-y-2">
                {stats.tools.top_tools.map((t) => (
                  <HorizontalBar key={t.name} name={t.name} count={t.count} max={maxToolCount} />
                ))}
              </div>
            </div>
          )}
          ...
          {stats.models.length > 0 && (
            <div className="card px-4 py-3">
              ...
              <div className="space-y-2">
                {stats.models.map((m) => (
                  <HorizontalBar key={m.model} name={m.model} count={m.sessions} max={maxModelCount} />
                ))}
              </div>
            </div>
          )}
              ...
              <div key={p.project} className="flex items-center gap-3">
                ...
                <div className="w-32 rounded-sm overflow-hidden" style={{ height: 6, background: 'rgba(255,255,255,0.06)' }}>
                  <div
                    ...
                    style={{
                      width: `${(p.sessions / maxProjectSessions) * 100}%`,
                      background: 'var(--accent)',
                      opacity: 0.7,
                    }}
                  />
                </div>
                <span className="text-xs tabular-nums w-16 text-right" style={{ color: 'var(--text-tertiary)' }}>
                  {p.sessions} sess · {p.total_tool_calls} tools
                </span>
              </div>
            ))}
          </div>
