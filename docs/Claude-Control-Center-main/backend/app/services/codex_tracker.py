...
Scans all session JSONL files under ~/.claude/projects/ for Agent tool_use
blocks where subagent_type == "codex:codex-rescue". Pairs each invocation
with its subsequent tool_result to capture output and duration.

Records are stored in backend/data/codex_usages.json.
"""
...
# Where we persist the scan results — inside the project repo, not ~/.claude
_DATA_FILE = Path(__file__).parent.parent.parent / "data" / "codex_usages.json"
...
def _scan_file(jsonl_path: Path, project_dirname: str) -> list[dict]:
    """Return Codex invocation records from a single JSONL file."""
    records: list[dict] = []

    # First pass: collect all messages
    messages = list(_iter_raw(jsonl_path))

    # Build a map of tool_use_id → index for fast result lookup
    # We need: assistant messages with Agent tool_use (subagent_type=codex:codex-rescue)
    # followed by user messages with tool_result referencing that id.

    # Index tool_result blocks by tool_use_id
    result_map: dict[str, dict] = {}  # tool_use_id → {timestamp, content, is_error}
    for msg in messages:
    ...
    for msg in messages:
        ...
        # Grab cwd/gitBranch from first assistant message that has them
        if cwd is None:
            cwd = msg.get("cwd")
        ...
        for block in content:
            ...
            if inp.get("subagent_type") != "codex:codex-rescue":
                continue
...
def scan_all() -> list[dict]:
    """Scan every session JSONL in ~/.claude/projects/ and return all Codex records."""
...
def scan_all_background() -> None:
    ...
    t = threading.Thread(target=scan_all, daemon=True, name="codex-scanner")
    t.start()
