...
import { Sparkles, RefreshCw, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Clock, CheckCircle, XCircle, HelpCircle } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { fetchAdvisorUsages, fetchAdvisorStats, triggerAdvisorScan } from '../api/advisor';
import type { AdvisorUsage } from '../api/advisor';
...
function StatusIcon({ status }: { status: AdvisorUsage['status'] }) {
...
function UsageRow({ usage }: { usage: AdvisorUsage }) {
  ...
  return (
    <>
      <div
        ...
        style={{
          gridTemplateColumns: '1.5fr 1fr 1fr 80px 80px 24px',
          borderBottom: '1px solid var(--border)',
        }}
        onClick={() => setExpanded((v) => !v)}
      >
        <span className="text-xs font-mono truncate" style={{ color: 'var(--text-primary)' }} title={usage.advice_text}>
          {usage.advice_text?.slice(0, 100) || '—'}
        </span>
      ...
      {expanded && (
        ...
        >
          <div className="grid grid-cols-2 gap-4 text-xs">
            ...
            {usage.executor_model && (
              <div>
                <span className="font-medium uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>Executor</span>
                <div className="font-mono mt-0.5" style={{ color: 'var(--text-secondary)' }}>{usage.executor_model}</div>
              </div>
            )}
            {usage.advisor_model && (
              <div>
                <span className="font-medium uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>Advisor</span>
                <div className="font-mono mt-0.5" style={{ color: 'var(--text-secondary)' }}>{usage.advisor_model}</div>
              </div>
            )}
            {usage.git_branch && (
              <div>
          ...
          {usage.advice_text && (
            <div>
              ...
              <div
                className="text-xs rounded p-3 overflow-auto advisor-advice-body"
                style={{
                  maxHeight: 400,
...
export function AdvisorPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: fetchAdvisorStats,
    staleTime: 60_000,
  });

  const { data, isLoading } = useQuery({
    queryKey: queryKeys.advisorUsages(page),
    queryFn: () => fetchAdvisorUsages(page, PAGE_SIZE),
    staleTime: 60_000,
  });

  const { mutate: rescan, isPending: scanning } = useMutation({
    mutationFn: triggerAdvisorScan,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.advisorStats() });
      queryClient.invalidateQueries({ queryKey: queryKeys.advisorUsages() });
    },
  });
  ...
  return (
    <div className="p-6 flex flex-col h-full gap-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Total calls" value={statsLoading ? '...' : (stats?.total ?? 0)} />
        <StatCard label="Model pairs" value={statsLoading ? '...' : (stats?.model_pairs.length ?? 0)} />
        <StatCard
          label="Avg duration"
      ...
      {/* Chart + breakdowns */}
      {stats && (stats.by_week.length > 0 || stats.model_pairs.length > 0 || stats.projects.length > 0) && (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          {stats.by_week.length > 0 && <WeekChart data={stats.by_week} />}
          {stats.model_pairs.length > 0 && (
            <div className="card px-4 py-3">
              <div className="text-xs font-medium mb-3 uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>
                By model pair
              </div>
              <div className="space-y-2">
                {stats.model_pairs.slice(0, 6).map((p) => (
          ...
          <div className="flex-1 overflow-auto">
            {(!data || data.items.length === 0) ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 py-16">
                ...
                <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>No advisor calls found yet.</p>
                <button
